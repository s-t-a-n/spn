...
// flag make sure that no two concurrent shutdowns can occur
static atomic_t g_shutdown_requested = ATOMIC_INIT(0);
...
ShutdownManager* set_shutdown_manager(ShutdownManager* mgr) {
    ...
    MLOG_DBG(spn_system, "shutdown manager set: %p", mgr);
    return old;
}
...
void request_shutdown(shutdown_reason reason) {
    ...
    if (atomic_cas(&g_shutdown_requested, 0, 1)) {
        _shutdown_work.reason = reason;
        k_work_schedule(&_shutdown_work.work, K_NO_WAIT);
    } else {
        MLOG_WRN(spn_system, "shutdown already in progress, ignored reason %i", static_cast<int>(reason));
    }
}
...
ExceptionHandler* set_exception_handler(ExceptionHandler* handler) {
    ...
    MLOG_DBG(spn_system, "exception handler set: %p", handler);
    return old;
}
